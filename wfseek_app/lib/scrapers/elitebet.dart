...
// EliteBet Nigeria – elitebet.ng API
class EliteBetScraper extends ScraperBase {
  EliteBetScraper() : super(bookmakerById('elitebet')!);
  ...
  Future<List<Map<String, dynamic>>> getOdds(String sport, String market) async {
    ...
    try {
      final r = await get('/api/events', query: {'sport': sid, 'market': _mkt(market), 'count': '200', 'status': 'upcoming'});
  ...
  Future<List<Map<String, dynamic>>> _tryAlt(String sid, String sport, String market) async {
    try {
      final r = await get('/api/sport/$sid/prematch', query: {'marketType': _mkt(market), 'limit': '200', 'lang': 'en'});
