...
// BC.Game – crypto sportsbook; bc.game sports API
class BCGameScraper extends ScraperBase {
  BCGameScraper() : super(bookmakerById('bcgame')!);
  ...
  Future<List<Map<String, dynamic>>> getOdds(String sport, String market) async {
    ...
    try {
      final r = await get('/api/v1/sport/events', query: {'sport': slug, 'market': _mkt(market), 'count': '200', 'status': 'prematch'});
      final body = r.data;
  ...
  Future<List<Map<String, dynamic>>> _tryAlt(String slug, String sport, String market) async {
    try {
      final r = await get('/sport/api/v2/$slug/matches', query: {'market': _mkt(market), 'limit': '200'});
      final events = r.data?['matches']??r.data?['events']??r.data?['data']; if (events is! List) return [];
      return _parse(events, sport, market);
    } catch (_) { return []; }
  }
