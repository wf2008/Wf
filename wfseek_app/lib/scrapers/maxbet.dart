...
class MaxBetScraper extends ScraperBase {
  ...
  Future<List<Map<String, dynamic>>> getOdds(String sport, String market) async {
    ...
    try {
      final r = await get('/api/events', query: {'sportId': sid, 'marketType': _mkt(market), 'status': 'prematch', 'count': '200'});
      final events = r.data?['events']??r.data?['data']??r.data?['matches']; if (events is! List) return _tryAlt(sid, sport, market);
  ...
  Future<List<Map<String, dynamic>>> _tryAlt(String sid, String sport, String market) async {
    try {
      final r = await get('/api/sport/$sid/prematch', query: {'market': _mkt(market), 'count': '200'});
  ...
  List<Map<String, dynamic>> _parse(List events, String sport, String market) {
    ...
    for (final ev in events) {
      ...
      final home=(ev['homeTeam']??ev['home']??ev['homeName']??'').toString().trim();
      final away=(ev['awayTeam']??ev['away']??ev['awayName']??'').toString().trim();
