...
// MerryBet Nigeria – merrybet.com API
class MerryBetScraper extends ScraperBase {
  MerryBetScraper() : super(bookmakerById('merrybet')!);
  ...
  Future<List<Map<String, dynamic>>> getOdds(String sport, String market) async {
    ...
    try {
      final r = await get('/api/events', query: {'sport': sid, 'marketType': _mkt(market), 'status': 'upcoming', 'count': '200', 'lang': 'en'});
  ...
  Future<List<Map<String, dynamic>>> _tryAlt(String sid, String sport, String market) async {
    try {
      final r = await get('/api/v1/sport/$sid/prematch', query: {'marketId': _mktId(market), 'count': '200'});
  ...
  List<Map<String, dynamic>> _parse(List events, String sport, String market) {
    ...
    for (final ev in events) {
      ...
      for (final mk in list) {
        ...
        final Map<String,dynamic> out2 = {}; final Map<String,List<double>> ouLines = {};
        for (final s in sels) {
          ...
          if (market=='1x2') { if (n.contains('home')||n=='1'||n=='w1') out2['Home']=v; else if (n.contains('draw')||n=='x') out2['Draw']=v; else if (n.contains('away')||n=='2'||n=='w2') out2['Away']=v; }
          else if (market=='moneyline'||market=='winner') { if (n.contains('home')||n=='1') out2['Home']=v; else if (n.contains('away')||n=='2') out2['Away']=v; }
          ...
          else if (market=='double_chance') { if (n.contains('1x')) out2['1X']=v; else if (n.contains('x2')) out2['X2']=v; else if (n.contains('12')) out2['12']=v; }
          else if (market=='btts') { if (n.contains('yes')||n.contains('gg')) out2['Yes']=v; else if (n.contains('no')||n.contains('ng')) out2['No']=v; }
        }
        ...
        else if (out2.length>=2) out.add({'id':'${id}_$market','home_team':home,'away_team':away,'league':league,'category':'','sport':sport,'market':market,'market_detail':market,'outcomes':out2});
      }
    }
    return out;
  }
