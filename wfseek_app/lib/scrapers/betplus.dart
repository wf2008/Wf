...
// BetPlus Nigeria – betplus.ng API
class BetPlusScraper extends ScraperBase {
  ...
  Future<List<Map<String, dynamic>>> _tryAlt(String sid, String sport, String market) async {
    try {
      final r = await get('/api/v1/sport/$sid/matches', query: {'marketType': _mkt(market), 'limit': '200'});
      final events = r.data?['events']??r.data?['data']??r.data?['matches']; if (events is! List) return [];
      return _parse(events, sport, market);
    } catch (_) { return []; }
  }
