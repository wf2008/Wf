...
// NaijaBet – naijabet.com API
class NaijaBetScraper extends ScraperBase {
  NaijaBetScraper() : super(bookmakerById('naijabet')!);
  ...
  Future<List<Map<String, dynamic>>> getOdds(String sport, String market) async {
    ...
    try {
      final r = await get('/api/sports/events', query: {'sportId': sid, 'market': _mkt(market), 'count': '200', 'status': 'upcoming', 'lang': 'en'});
      final events = r.data?['events']??r.data?['data']??r.data?['matches']??r.data?['result'];
      if (events is! List) return _tryAlt(sid, sport, market);
  ...
  Future<List<Map<String, dynamic>>> _tryAlt(String sid, String sport, String market) async {
    try {
      final r = await get('/api/v2/events/prematch', query: {'sport': sid, 'marketType': _mkt(market), 'limit': '200'});
  ...
  List<Map<String, dynamic>> _parse(List events, String sport, String market) {
    ...
    for (final ev in events) {
      ...
      final league=(ev['league']??ev['competition']??ev['tournament']??ev['leagueName']??ev['tournamentName']??'').toString();
  ...
  String _mkt(String m) { switch(m) { case '1x2': return '1X2'; case 'over_under': return 'OU'; case 'double_chance': return 'DC'; case 'btts': return 'GG'; case 'moneyline': return 'ML'; case 'winner': return 'WN'; default: return m; } }
