...
// Parimatch Nigeria – parimatch.ng API
class ParimatchScraper extends ScraperBase {
  ParimatchScraper() : super(bookmakerById('parimatch')!);
  ...
  Future<List<Map<String, dynamic>>> getOdds(String sport, String market) async {
    ...
    try {
      final r = await get('/api/line/sport/$slug', query: {'market': _mkt(market), 'count': '200', 'lang': 'en', 'currency': 'NGN'});
      final body = r.data;
      final events = body?['data']??body?['events']??body?['sports']?[0]?['events']??body?['result'];
  ...
  Future<List<Map<String, dynamic>>> _tryAlt(String slug, String sport, String market) async {
    try {
      // Parimatch uses LineFeed-like API
      final r = await get('/LineFeed/GetLine', query: {'sport': slug, 'market': _mkt(market), 'count': '200', 'lng': 'en'});
      final events = r.data?['events']??r.data?['Value']??r.data?['data']; if (events is! List) return [];
      return _parse(events, sport, market);
    } catch (_) { return []; }
  }
  ...
  List<Map<String, dynamic>> _parse(List events, String sport, String market) {
    ...
    for (final ev in events) {
      ...
      final id=(ev['id']??ev['eventId']??ev['Id']??ev['matchId']??'').toString();
      final home=(ev['homeTeam']??ev['home']??ev['Team1']??ev['homeName']??ev['HomeTeam']??'').toString().trim();
      final away=(ev['awayTeam']??ev['away']??ev['Team2']??ev['awayName']??ev['AwayTeam']??'').toString().trim();
      if (home.isEmpty||away.isEmpty) continue;
      final league=(ev['league']??ev['competition']??ev['League']??ev['tournament']??ev['leagueName']??'').toString();
      final mks=ev['markets']??ev['odds']??ev['Markets']??ev['bet']; if (mks==null) continue;
      ...
      for (final mk in list) {
        ...
        for (final s in sels) {
          ...
          final n=(s['name']??s['label']??s['type']??s['Name']??'').toString().toLowerCase().trim();
          final v=_d(s['price']??s['odds']??s['value']??s['odd']??s['Odds']??s['Price']); if (v==null) continue;
  ...
  String _mkt(String m) { switch(m) { case '1x2': return '1x2'; case 'over_under': return 'total'; case 'double_chance': return 'double_chance'; case 'btts': return 'both_goals'; case 'moneyline': return 'winner'; case 'winner': return 'winner'; default: return m; } }
