...
// BetBonanza Nigeria – betbonanza.com API
class BetBonanzaScraper extends ScraperBase {
  ...
  Future<List<Map<String, dynamic>>> getOdds(String sport, String market) async {
    ...
    try {
      final r = await get('/api/events', query: {'sportId': sid, 'market': _mkt(market), 'count': '200', 'status': 'upcoming', 'lang': 'en'});
  ...
  Future<List<Map<String, dynamic>>> _tryAlt(String sid, String sport, String market) async {
    try {
      final r = await get('/api/sport/$sid/prematch', query: {'marketType': _mkt(market), 'limit': '200'});
