...
// AccessBet Nigeria – accessbet.com API
class AccessBetScraper extends ScraperBase {
  AccessBetScraper() : super(bookmakerById('accessbet')!);
  ...
  Future<List<Map<String, dynamic>>> getOdds(String sport, String market) async {
    ...
    try {
      final r = await get('/api/sports/v1/events', query: {'sport': sid, 'market': _mkt(market), 'count': '200', 'type': 'prematch'});
      final events = r.data?['data']??r.data?['events']??r.data?['matches']; if (events is! List) return _tryAlt(sid, sport, market);
  ...
  Future<List<Map<String, dynamic>>> _tryAlt(String sid, String sport, String market) async {
    try {
      final r = await get('/api/events/prematch', query: {'sportId': sid, 'marketType': _mkt(market), 'limit': '200'});
