...
// Supabet Nigeria – supabet.com API
class SupabetScraper extends ScraperBase {
  ...
  Future<List<Map<String, dynamic>>> getOdds(String sport, String market) async {
    ...
    try {
      final r = await get('/api/sport/$sid/events', query: {'market': _mkt(market), 'count': '200', 'type': 'upcoming'});
  ...
  Future<List<Map<String, dynamic>>> _tryAlt(String sid, String sport, String market) async {
    try {
      final r = await get('/api/events/prematch', query: {'sportId': sid, 'marketId': _mktId(market), 'limit': '200'});
